...
mod tests {
    ...
    use iroh::{endpoint::Connection, Endpoint, EndpointAddr, EndpointId, SecretKey};
    ...
    async fn net_smoke() -> Result<()> {
        ...
        let (ep_alfie, node_id_alfie, _) = create_endpoint(&mut rng, &disco).await?;
        let (ep_betty, node_id_betty, addr_betty) = create_endpoint(&mut rng, &disco).await?;

        let start = Instant::now();
        let mut expected_entries = BTreeSet::new();

        let handle_alfie = ActorHandle::spawn_memory(iroh_blobs::store::mem::MemStore::default().into(), node_id_alfie);
        let handle_betty = ActorHandle::spawn_memory(iroh_blobs::store::mem::MemStore::default().into(), node_id_betty);
        ...
        let (conn_alfie, conn_betty) = tokio::join!(
            ...
            async { ep_betty.accept().await.unwrap().await.unwrap() }
        );
    ...
    /// This bridges the rand_core 0.6 / 0.9 version gap.
    fn generate_secret_key(rng: &mut impl rand_core::CryptoRngCore) -> SecretKey {
    ...
    pub async fn create_endpoint(
        ...
        let ep = Endpoint::empty_builder(iroh::RelayMode::Disabled)
            .secret_key(generate_secret_key(rng))
        ...
        let addr = ep.addr();
        let node_id = ep.id();
        disco.add_endpoint_info(addr.clone());
        Ok((ep, node_id, addr))
    }
