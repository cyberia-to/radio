...
impl Builder {
    ...
    pub fn spawn(self, endpoint: Endpoint) -> Gossip {
        ...
        endpoint.address_lookup().expect("endpoint closed").add(address_lookup.clone());
...
pub(crate) mod test {
    ...
    pub(crate) async fn create_endpoint(
        ...
        if let Some(memory_lookup) = memory_lookup {
            ep.address_lookup().expect("endpoint closed").add(memory_lookup);
        }
        ep.online().await;
        Ok(ep)
    }
    ...
    async fn subscription_cleanup() -> Result {
        ...
        actor.endpoint.address_lookup().expect("endpoint closed").add(memory_lookup);
    ...
    async fn can_reconnect() -> Result {
        ...
        ep2.address_lookup().expect("endpoint closed").add(memory_lookup.clone());
        let go2_task = async move {
        ...
        let addr2 = EndpointAddr::new(endpoint_id2).with_relay_url(relay_url);
        memory_lookup.add_endpoint_info(addr2);
        ep1.address_lookup().expect("endpoint closed").add(memory_lookup);
    ...
    async fn can_die_and_reconnect() -> Result {
        ...
        async fn broadcast_once(
            ...
            router.endpoint().address_lookup().expect("endpoint closed").add(memory_lookup);
    ...
    async fn gossip_change_alpn() -> n0_error::Result<()> {
        ...
        let memory_lookup = MemoryLookup::new();
        memory_lookup.add_endpoint_info(addr1);
        router2.endpoint().address_lookup().expect("endpoint closed").add(memory_lookup);
    ...
    async fn gossip_rely_on_gossip_address_lookup() -> n0_error::Result<()> {
        ...
        r2.endpoint().address_lookup().expect("endpoint closed").add(lookup.clone());
        ...
        r3.endpoint().address_lookup().expect("endpoint closed").add(lookup.clone());
    ...
    async fn topic_stays_alive_after_sender_drop() -> n0_error::Result<()> {
        ...
        router2.endpoint().address_lookup().expect("endpoint closed").add(mem_lookup);
