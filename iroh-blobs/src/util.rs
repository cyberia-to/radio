        ...
        for item in tree.post_order_chunks_iter() {
            match item {
                ...
                BaoChunk::Leaf {
                    ...
                    if let Err(err) = progress.send(ChunkNum(start_chunk)).await {
                        return Ok(Err(err));
                    }
                    let buf = &mut buffer[..size];
                    data.read_exact(buf)?;
                    let hash = hash_subtree(start_chunk, buf, is_root, block_bytes);
                    stack.push(hash);
                }
            }
        }
    ...
    mod tests {
        ...
        async fn init_outboard_with_progress() -> TestResult<()> {
            for size in [1024 * 18 + 1] {
                ...
                // Compute expected root with our init_outboard
                init_outboard(data.as_ref(), &mut o1, &mut Drain).await??;
                // Verify against Hash::new
                let expected = Hash::new(&data);
                let actual: Hash = o1.root.into();
...
pub(crate) mod sink {
    ...
    impl<P, F> std::fmt::Debug for WithMapErr<P, F> {
        fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
            f.debug_struct("WithMapErr").finish()
        }
    }
    ...
    impl<P, F> std::fmt::Debug for WithMap<P, F> {
        fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
            f.debug_struct("WithMap").finish()
        }
    }
