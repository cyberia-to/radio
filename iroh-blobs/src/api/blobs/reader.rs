...
mod tests {
    ...
    async fn reader_partial(blobs: &Blobs) -> TestResult<()> {
        ...
        for size in INTERESTING_SIZES {
            ...
            // Importing chunk 0 with block_size=4 imports the entire first block.
            // The available bytes are min(size, block_bytes).
            let available = size.min(block_bytes);
            // read the available data — this should work!
            {
            ...
            if size > block_bytes {
                ...
                // read crossing the end of the available range - should return an error
                // despite the first bytes being valid.
