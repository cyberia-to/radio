...
impl fmt::Debug for GetProgress {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("GetProgress").finish()
    }
}
...
impl fmt::Debug for PushProgress {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("PushProgress").finish()
    }
}
...
mod tests {
    use cyber_bao::{ChunkNum, ChunkRanges};
    ...
    async fn test_observe_partial(blobs: &Blobs) -> TestResult<()> {
        ...
        for size in sizes {
            ...
            if size > block_bytes {
                // With block-level granularity, importing chunk 0 imports
                // the entire first block (chunks 0..chunks_per_block).
                let expected = ChunkRanges::from(ChunkNum(0)..ChunkNum(chunks_per_block));
                assert_eq!(bitfield.ranges, expected, "size={}", size);
            } else {
                // Data fits within one block — all chunks are available
                assert_eq!(bitfield.ranges, ChunkRanges::all(), "size={}", size);
            }
        }
        Ok(())
    }
