...
//!
//!   Note: Validation using Poseidon2 is extremely fast, so in almost all scenarios the validation
...
//!   chunk group of 16KiB or 16 chunks. The worst case overhead when doing range requests
...
//!
//! Ranges are always given in terms of 1024 byte chunks, *not* in terms
//! of bytes or chunk groups. The reason for this is that chunks are the fundamental
//! unit of hashing in the BAO tree. Addressing anything smaller than a chunk is not
...
//! and the [cyber-bao](../cyber_bao) crate. The cyber-bao crate
//! implements the bao tree structure using Poseidon2 over Goldilocks field,
//! with support for combining multiple chunks to chunk groups for efficiency.
//!
