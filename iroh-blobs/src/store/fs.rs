...
impl HashSpecificCommand for ExportBaoMsg {
    ...
    async fn on_error(self, arg: SpawnArg<EmParams>) {
        ...
        self.tx
            .send(EncodedItem::Error(cyber_bao::io::EncodeError::Io(err)))
            .await
            .ok();
    }
}
...
impl EntityApi for HashContext {
    ...
    async fn export_bao(&self, mut cmd: ExportBaoMsg) {
        ...
        if let Err(cause) = export_bao_impl(self, cmd.inner, &mut cmd.tx).await {
            ...
            cmd.tx
                .send(cyber_bao::io::EncodeError::Io(cause).into())
                .await
                .ok();
        }
    }
...
pub mod tests {
    ...
    use cyber_bao::{io::round_up_to_chunks_groups, ChunkRanges};
    ...
    pub fn round_up_request(size: u64, ranges: &ChunkRanges) -> ChunkRanges {
        ...
        let ranges = if !data_range.intersects(ranges) && !ranges.is_empty() {
            if last_chunk == ChunkNum(0) {
                ChunkRanges::all()
            } else {
                ChunkRanges::from(last_chunk - 1u64..)
            }
        } else {
            ranges.clone()
        };
    ...
    ///
    /// We don't really care about the content, since we assume the hash function works.
    ...
    async fn smoke() -> TestResult<()> {
        ...
        for (_hash, _cyber_bao) in bao_by_hash {
            // let mut reader = Cursor::new(cyber_bao);
            ...
            //     let item = item?;
            // }
            // store.import_bao_bytes(hash, ChunkRanges::all(), cyber_bao.into()).await?;
        }
        Ok(())
    }
